...
static int16_t _24cxx_write_page(_24cxx_dev_t *pdev, uint32_t addr, uint8_t *pbuf, uint32_t size)
{
	...
	ret = pdev->i2c_send_then_send(slave_addr, buf, buf_size, pbuf, size);
...
static int16_t _24cxx_write_continue(_24cxx_dev_t *pdev, uint32_t addr, uint8_t *pbuf, uint32_t size)
{
  uint8_t offset = 0;
	...
	{/* 24c32-24c1024 */
		...
		buf[0] = (addr >>8)& 0xff;
		buf[1] = addr & 0xff;
		ret = pdev->i2c_send_then_send(slave_addr, buf, 2, pbuf, size);
	}
	else
	{/* 24c01-24c16 */
		while (size > 0)
		{
			...
			ret = pdev->i2c_send_then_send(slave_addr, buf, 1, pbuf, write_size);
			if (ret != _24CXX_OK)
			{
				break;
			}
...
{  
	...
	ret = pdev->i2c_send_then_recv(slave_addr, buf, buf_size, pbuf, size);
