# 1 "../User/BSP/dev_usart.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 387 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../User/BSP/dev_usart.c" 2
...
# 2 "../User/BSP/dev_usart.c" 2
         ...
         uint8_t usart1_rx_buf[(1024U)];
         uint8_t usart1_dmarx_buf[( 128U)];
         uint8_t usart1_tx_buf[(1024U)];
         uint8_t usart1_dmatx_buf[( 128U)];


         uint8_t usart2_rx_buf[(1024U)];
         uint8_t usart2_dmarx_buf[( 128U)];
         uint8_t usart2_tx_buf[(1024U)];
         uint8_t usart2_dmatx_buf[( 128U)];


         UsartDevice_T s_usart_dev[DEV_USART_COUNT] = {0};



static int usart_dmatx_check_busy(uint8_t usart_id);
static uint16_t usart_dmarx_remain_size(uint8_t usart_id);
# 35 "../User/BSP/dev_usart.c"
void usart_device_init(uint8_t usart_id) {
  switch (usart_id) {
    case DEV_USART1:

      fifo_init(&s_usart_dev[DEV_USART1].rx_fifo, usart1_rx_buf, (1024U));
      fifo_init(&s_usart_dev[DEV_USART1].tx_fifo, usart1_tx_buf, (1024U));
      s_usart_dev[DEV_USART1].dmarx_buf = usart1_dmarx_buf;
      s_usart_dev[DEV_USART1].dmarx_buf_size = ( 128U);
      s_usart_dev[DEV_USART1].dmatx_buf = usart1_dmatx_buf;
      s_usart_dev[DEV_USART1].dmatx_buf_size = ( 128U);
      s_usart_dev[DEV_USART1].dmarx_buf_read = 0;
      s_usart_dev[DEV_USART1].status = USART_DMA_IDLE;

      bsp_usart1_init(usart1_dmarx_buf, ( 128U));
      break;

    case DEV_USART2:

      fifo_init(&s_usart_dev[DEV_USART2].rx_fifo, usart2_rx_buf, (1024U));
      fifo_init(&s_usart_dev[DEV_USART2].tx_fifo, usart2_tx_buf, (1024U));
      s_usart_dev[DEV_USART2].dmarx_buf = usart2_dmarx_buf;
      s_usart_dev[DEV_USART2].dmarx_buf_size = ( 128U);
      s_usart_dev[DEV_USART2].dmatx_buf = usart2_dmatx_buf;
      s_usart_dev[DEV_USART2].dmatx_buf_size = ( 128U);
      s_usart_dev[DEV_USART2].dmarx_buf_read = 0;
      s_usart_dev[DEV_USART2].status = USART_DMA_IDLE;

      bsp_usart2_init(usart2_dmarx_buf, ( 128U));
      break;
...
static int usart_dmatx_check_busy(uint8_t usart_id) {
  switch (usart_id) {
    case DEV_USART1:
      return (s_usart_dev[usart_id].status || !LL_USART_IsActiveFlag_TC(((USART_TypeDef *) ((0x40000000UL + 0x00010000UL) + 0x1000UL))));
      break;

    case DEV_USART2:
      return (s_usart_dev[usart_id].status || !LL_USART_IsActiveFlag_TC(((USART_TypeDef *) (0x40000000UL + 0x4400UL))));
      break;
