...
int main(void)
{
  ...
  LL_APB2_GRP1_EnableClock((0x1UL << (14U)));
  LL_APB1_GRP1_EnableClock((0x1UL << (28U)));


  __NVIC_SetPriorityGrouping(((uint32_t)0x00000005));
 ...
 while (1) {
    ...
    uint8_t recv_tmp;
    if (usart_read(DEV_USART1, &recv_tmp, 1)) {
      usart_write(DEV_USART1, &recv_tmp, 1);
    }
...
void SystemClock_Config(void)
{
  LL_FLASH_SetLatency(0x00000005U);
  while(LL_FLASH_GetLatency()!= 0x00000005U)
  {
  }
  LL_PWR_SetRegulVoltageScaling(((0x1UL << (14U))));
  LL_RCC_HSE_Enable();
  ...
  LL_RCC_PLL_ConfigDomain_SYS((0x1UL << (22U)), ((0x08UL << (0U))), 336, 0x00000000U);
  LL_RCC_PLL_Enable();
  ...
  LL_RCC_SetAPB1Prescaler(0x00001400U);
  LL_RCC_SetAPB2Prescaler(0x00008000U);
